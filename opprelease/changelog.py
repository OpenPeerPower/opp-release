...
LINK_DEF_USER = "[@{0}]: {1}"
LINK_DEF_PR = "[#{0}]: {1}"
GITHUB_LINK_DEF_DOC = "[{0} docs]: https://www.open-peer-power.io/integrations/{0}/"
...
...don't hesitate to use our very active [forums](https://community.open-peer-power.io/) or join us for a little [chat](https://discord.gg/c5DvZ4e).

## Reporting Issues

Experiencing issues introduced by this release? Please report them in our [issue tracker](https://github.com/openpeerpower/openpeerpower/issues). Make sure to fill in all fields of the issue template.
...
def automation_link(platform, website_tags):
    ...
    if platform == "automation.openpeerpower":
        val = "open-peer-power"
    ...
    if website_tags:
        ...
        format = "[{} docs]: https://www.open-peer-power.io/docs/automation/trigger/#{}-trigger"
...
def generate(release, prs, *, website_tags):
    ...
    website_tags: boolean if we should include tags for open-peer-power.io
    """
